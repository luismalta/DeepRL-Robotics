...
parser = argparse.ArgumentParser(description='Dueling DQN agent')
...
nb_steps = 10
nb_episodes = 100
log_dir = './logs/Dueling_DQN/log_{}'.format(datetime.now().strftime('%d-%m-%Y_%H:%M'))
...
def train():
    ...
    dqn.save_weights('./models/Dueling_DQN/{}_steps_{}_weights.h5f'.format(nb_steps,datetime.now().strftime('%d-%m-%Y_%H:%M')), overwrite=False)
