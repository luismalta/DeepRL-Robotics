...
nb_steps = 10
nb_episodes = 100
log_dir = './logs/Double_DQN/log_{}'.format(datetime.now().strftime('%d-%m-%Y_%H:%M'))

if(args.test == True and args.weight == None):
    print("Provide the weight file name")
    sys.exit()
else:
    weight_dir = 'models/Double_DQN/{}.h5f'.format(args.weight)
...
def train():
    ...
    dqn.save_weights('./models/Double_DQN/{}_steps_{}_weights.h5f'.format(nb_steps,datetime.now().strftime('%d-%m-%Y_%H:%M')), overwrite=False)
